...
class ResNet(tf.keras.Model):
    ...
    def resnet_block(self, inputs, filters, kernel_size, strides, padding):
        shortcut = inputs
    ...
    def call(self, rdf_shape, num_classes, num_filters=64, num_blocks=[2, 2, 2, 2], dropout_rate=0.2):
        x = layers.Conv1D(filters=num_filters, kernel_size=7, strides=2, padding='same',
                          kernel_regularizer=regularizers.l2(weight_decay))(rdf_input)
        x = layers.BatchNormalization()(x)
        x = layers.ReLU()(x)
        ...
        for i, num_block in enumerate(num_blocks):
            for j in range(num_block):
                ...
                x = self.resnet_block(x, filters=num_filters * (2 ** i), kernel_size=3, strides=strides,
